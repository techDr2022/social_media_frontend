...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!authHeader) {
      console.error('[GMB Connect API] No authorization header');
    ...
    const targetUrl = `${cleanBackendUrl}/api/v1/social-accounts/connect/gmb`;
    
    console.log(`[GMB Connect API] Starting request`);
    console.log(`[GMB Connect API] Backend URL: ${cleanBackendUrl}`);
    console.log(`[GMB Connect API] Target URL: ${targetUrl}`);
    console.log(`[GMB Connect API] Has auth header: ${!!authHeader}`);
    ...
    try {
      ...
      console.log(`[GMB Connect API] Backend response status: ${response.status}`);
      console.log(`[GMB Connect API] Backend response headers:`, Object.fromEntries(response.headers.entries()));
    } catch (fetchError: any) {
      console.error('[GMB Connect API] Fetch error caught:', fetchError);
      console.error('[GMB Connect API] Error name:', fetchError.name);
      console.error('[GMB Connect API] Error message:', fetchError.message);
      console.error('[GMB Connect API] Error stack:', fetchError.stack);
    ...
    if (response.status >= 300 && response.status < 400) {
      ...
      if (redirectUrl) {
        console.log('[GMB Connect API] Redirecting to:', redirectUrl);
    ...
    console.error('[GMB Connect API] Unexpected response:', response.status, errorText);
    return NextResponse.json(
