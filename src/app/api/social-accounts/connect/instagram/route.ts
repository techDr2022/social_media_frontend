...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!authHeader) {
      console.error('[Instagram Connect API] No authorization header');
    ...
    const targetUrl = `${cleanBackendUrl}/api/v1/social-accounts/connect/instagram`;
    ...
    try {
      ...
      console.log(`[Instagram Connect API] Backend response status: ${response.status}`);
      console.log(`[Instagram Connect API] Backend response headers:`, Object.fromEntries(response.headers.entries()));
    } catch (fetchError: any) {
      console.error('[Instagram Connect API] Fetch error caught:', fetchError);
      console.error('[Instagram Connect API] Error name:', fetchError.name);
      console.error('[Instagram Connect API] Error message:', fetchError.message);
      console.error('[Instagram Connect API] Error stack:', fetchError.stack);
    ...
    if (response.status >= 300 && response.status < 400) {
      ...
      if (redirectUrl) {
        console.log('[Instagram Connect API] Redirecting to:', redirectUrl);
    ...
    console.error('[Instagram Connect API] Unexpected response:', response.status, errorText);
    return NextResponse.json(
