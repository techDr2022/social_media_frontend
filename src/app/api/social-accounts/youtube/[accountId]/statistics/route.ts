...
export async function GET(
  ...
  try {
    ...
    const backendPath = `${cleanBackendUrl}/api/v1/social-accounts/youtube/${accountId}/statistics`;
    console.log(`[YouTube Statistics API] Fetching statistics for account: ${accountId} from ${backendPath}`);
    ...
    try {
      ...
      console.error('[YouTube Statistics API] Fetch error:', fetchError);
      return NextResponse.json(
    ...
    try {
      ...
      console.error('[YouTube Statistics API] JSON parse error:', jsonError);
      return NextResponse.json(
    ...
    if (!response.ok) {
      const errorMessage = data?.message || data?.error || `Failed to load statistics (${response.status})`;
      return NextResponse.json(
        { error: errorMessage },
